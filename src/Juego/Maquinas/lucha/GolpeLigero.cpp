...
namespace IVJ
{
    GolpeLigero::GolpeLigero(int max_frames, float frame_rate)
    ...
    FSM* GolpeLigero::onInputs(const CE::IControl& control)
    {
        (void)control;
        //al terminar la animación regresar a idle
        if(animacion_terminada)
            return new IdleLucha(2, 0.12f);
        return nullptr;
    }

    void GolpeLigero::onEntrar(const Entidad& obj)
    {
        ...
        //consumir el input para que no se repita al volver a idle
        auto control = obj.getComponente<CE::IControl>();
        if(control) control->punch = false;

        //mostrar primer frame de la animación inmediatamente
        sprite->setTextureRect(
            sf::IntRect(
                sf::Vector2i(0, 3 * s_h), //row 4 (golpe)
                sf::Vector2i(s_w, s_h)
            )
        );
        id_frame = 1; // Ya mostramos el 0, pasar al 1
    }

    void GolpeLigero::onSalir(const Entidad& obj)
    {
        auto combate = obj.getComponente<ICombate>();
        if(combate) {
    ...
    void GolpeLigero::onUpdate(const Entidad& obj, float dt)
    {
        (void)obj;
        cur_frame_time -= dt;
        if(cur_frame_time <= 0)
        {
            ...
            sprite->setTextureRect(
                sf::IntRect(
                    sf::Vector2i(s_w * (id_frame % max_frames), 3 * s_h), //row 2
                    sf::Vector2i(s_w, s_h)
                )
            );
            id_frame++;
